...
async function test() {
  ...
  const funding = await Funding.deploy("0xc32a2dfee97e2babc90a2b5e6aef41e789ef2e13", 0);

  await funding.deployed();
  console.log("Greeter deployed to:", funding.address);
  const fundingRate = await funding.fundingRate()
  console.log("fundingRate: ", fromWei(fundingRate.toString()))
  const unitAccumulativeFunding = await funding.unitAccumulativeFunding()
